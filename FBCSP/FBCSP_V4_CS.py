...
Modified for cross subject, now no train-valid split inside the training loop
"""
...
class FBCSP_V4():
    ...
    def __init__(self, data_dict, fs, n_w = 2, n_features = 4, freqs_band = None, filter_order = 3, classifier = None, print_var = True, train_ratio=0.75):
        self.fs = fs
        self.trials_dict = data_dict # has only training set this time
        self.n_w = n_w
...
#         self.tmp_test = [test_data, test_label]
        ...
        self.train_score = scorer(self.classifier, train_data, train_labels)
        #self.valid_score = scorer(self.classifier, self.valid_data, self.valid_labels)
